...
| `publisher_jwks_url`                                       | the URL of the JSON Web Key Set (JWK Set) to use for validating publishers JWT, provided by identity providers such as Keycloak or AWS Cognito; a `file://` URL pointing to a local JWK Set JSON file is also accepted, in which case the file is read at provision time and rotating the keys requires a Caddy config reload (take precedence over `publisher_jwt`) |                        |
| `subscriber_jwks_url`                                      | the URL of the JSON Web Key Set (JWK Set) to use for validating subscribers JWT; a `file://` URL pointing to a local JWK Set JSON file is also accepted, in which case the file is read at provision time and rotating the keys requires a Caddy config reload (take precedence over `subscriber_jwt`)                                                               |                        |
...
| `read_header_timeout`      | the amount of time allowed to read request headers, set to `0s` to disable                                                                                                                                                                                                                                                                                                                   | `5s`                                                     |
